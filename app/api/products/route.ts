...
export async function GET() {
  try {
    const res = await query('SELECT id, sku, name, description, price, stock, category, image, min_stock AS "minStock" FROM products ORDER BY name ASC')
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!id || typeof delta !== 'number') {
      return NextResponse.json({ error: 'Product id and delta quantity are required' }, { status: 400 })
    }
