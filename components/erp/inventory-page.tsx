...
import { Plus, Minus, Search, Filter, Package, AlertTriangle } from 'lucide-react'
    ...
    <div className="glass-card-hover overflow-hidden group">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className={`text-sm font-medium ${isOutOfStock ? 'text-destructive' : isLowStock ? 'text-warning' : 'text-muted-foreground'}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-foreground">Inventory (WMS)</h1>
          <p className="text-muted-foreground mt-1">Manage your product stock levels</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/20">
            <Package className="w-4 h-4 text-destructive" />
            <span className="text-sm font-medium text-destructive">{outOfStockCount} Out of Stock</span>
          </div>
        ...
        <div className="flex items-center gap-4 text-sm">
          <span className="text-muted-foreground">
            Total Value: <span className="font-semibold text-foreground">
              ${products.reduce((sum, p) => sum + (p.price * p.stock), 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </span>
          </span>
        </div>
      </div>
